...
import { getKC1Logs, LogItemKC1 } from '@/services/kc1-service';

export default function MonitorClient1() {
  const [activeTab, setActiveTab] = useState<'error' | 'warning'>('error');
  const [data, setData] = useState<LogItemKC1[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const logs = await getKC1Logs(activeTab);
      setData(logs);
    } catch (err) {
      console.error(err);
    } finally {
