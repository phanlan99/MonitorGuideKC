...
import { getKC2DailyDetailedStats, DailyDetailStat } from '@/services/kc2-daily-stats';
import { Loader2, Calendar, AlertOctagon, AlertTriangle, Flame } from 'lucide-react';

export default function AnalysisDailyStats2() {
  const [data, setData] = useState<DailyDetailStat[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      const result = await getKC2DailyDetailedStats();
      setData(result);
      setLoading(false);
    };
    fetchData();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-10 bg-white rounded-xl border border-slate-200 mt-6">
        ...
        <span className="text-slate-500 font-medium">Đang tải phân tích chi tiết GuideKC #2...</span>
      </div>
    );
  }
          ...
          <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:border-purple-200 transition-colors">
            ...
            <div className="bg-slate-50 p-4 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <div className="bg-purple-100 p-2 rounded-lg"><Calendar className="w-5 h-5 text-purple-600" /></div>
                <div>
