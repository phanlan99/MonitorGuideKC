// src/components/AnalysisChartMonthly1.tsx
'use client';

import { useEffect, useState } from 'react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import { getKC1AnalysisMonthly } from '@/services/kc1-analysis-services'; // Đã đổi
import { Loader2, CalendarDays, Filter } from 'lucide-react';

// IMPORT COMPONENT BẢNG THỐNG KÊ CHI TIẾT
import AnalysisMonthlyStats1 from './AnalysisMonthlyStats1'; 
...
export default function AnalysisChartMonthly1() { // Đã đổi tên
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const result = await getKC1AnalysisMonthly(selectedYear); // Đã đổi hàm
        setData(result);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="w-full bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }} barGap={4}>
      ...
      <AnalysisMonthlyStats1 selectedYear={selectedYear} />
