...
import { getKC2WeeklyDetailedStats, WeeklyDetailStat, WeeklyStatsResponse } from '@/services/kc2-weekly-stats';
import { Loader2, CalendarDays, AlertOctagon, AlertTriangle, Siren, ChevronLeft, ChevronRight } from 'lucide-react';

export default function AnalysisWeeklyStats2({ selectedYear }: { selectedYear: number }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await getKC2WeeklyDetailedStats(selectedYear);
      setData(result);
      setCurrentPage(1); 
      setLoading(false);
    };
  ...
  if (loading || !data) {
    return (
      <div className="flex items-center justify-center p-10 bg-white rounded-xl border border-slate-200 mt-6">
        ...
        <span className="text-slate-500 font-medium">Đang tải phân tích chi tiết tuần GuideKC #2...</span>
      </div>
    );
  }
    ...
    <div className="mt-8 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        {currentWeeks.map((w, idx) => (
          <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:border-purple-200 transition-colors">
            ...
            <div className="bg-slate-50 p-4 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <div className="bg-purple-100 p-2 rounded-lg"><CalendarDays className="w-5 h-5 text-purple-600" /></div>
                <div>
