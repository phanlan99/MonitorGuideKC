...
import { getKC2MonthlyDetailedStats, MonthlyStatsResponse } from '@/services/kc2-monthly-stats';
import { Loader2, Calendar, AlertOctagon, AlertTriangle, ChevronLeft, ChevronRight, TrendingUp } from 'lucide-react';

export default function AnalysisMonthlyStats2({ selectedYear }: { selectedYear: number }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await getKC2MonthlyDetailedStats(selectedYear);
      setData(result);
      setCurrentPage(1); 
      setLoading(false);
    };
  ...
  if (loading || !data) {
    return (
      <div className="flex items-center justify-center p-10 bg-white rounded-xl border border-slate-200 mt-6">
        ...
        <span className="text-slate-500 font-medium">Đang tải phân tích chi tiết tháng năm {selectedYear} GuideKC #2...</span>
      </div>
    );
  }
    ...
    <div className="mt-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {currentMonths.map((m, idx) => (
          <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:border-purple-200 transition-colors">
